    ...
    public abstract class ExpressionNode
    {
        ...
        internal void EnsureReferenceInfo()
        {
            if (_objRefList == null)
            {
                ...
                foreach (var compObj in compObjects)
                {
                    ...
                    // Make it look like "SpriteVisual1"
                    string paramName = compObj.ToString();
                    paramName = $"{paramName.Substring(paramName.LastIndexOf('.') + 1)}{++id}";       // make sure the created param name doesn't overwrite a custom name
        ...
        internal void SetAllParameters(CompositionAnimation anim)
        {
            ...
            foreach (var constParam in _constParamMap)
            {
                ...
                {
                    throw new NotSupportedException("SetBooleanParameter");
